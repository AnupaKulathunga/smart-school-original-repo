...
class Course_mail_sms {
    ...
    public function publishsentmail($sender_details, $template, $subject,$chk_mail_sms) {
        ...
        $academic_class_ids_str = $this->getAcademicClassIds($sender_details);
        $academic_classes = explode(',', $academic_class_ids_str);

        foreach ($academic_classes as $academic_class_id) {
            $academic_class_id = trim($academic_class_id);
            if (empty($academic_class_id)) continue;

            $student_list = $this->CI->course_model->getStudentByAcademicClassID($academic_class_id);
    ...
    public function getpublishcontent($sender_details, $template) {
        ...
        // TVET: Remove {{section}} from templates — TVET has no sections
    ...
    /**
     * TVET: Get academic class IDs from sender_details.
     * Supports both new 'academic_class_ids' key and legacy 'class_section_id' key
     * for backward compatibility with payment controllers.
     */
    private function getAcademicClassIds($sender_details) {
        if (!empty($sender_details['academic_class_ids'])) {
            return $sender_details['academic_class_ids'];
        }
        if (!empty($sender_details['class_section_id'])) {
            return $sender_details['class_section_id'];
        }
        return '';
    }
