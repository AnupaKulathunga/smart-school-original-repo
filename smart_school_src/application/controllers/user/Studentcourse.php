...
class Studentcourse extends Student_Controller
{
    ...
    {  
        ...
        $academic_class_ids = array();
        if ($role == 'student' || $role == 'parent') {
            $userid = $this->result["student_id"];

            // Get student's academic class IDs from their enrolments
            $session_id = $this->setting[0]->session_id;
            $enrolments = $this->academic_enrolment_model->getStudentClasses($userid, $session_id);
            $academic_class_ids = array_map(function($e) { return $e->class_id; }, $enrolments);
            $data['academic_class_ids'] = $academic_class_ids;
        } elseif ($role == 'guest') {
            $userid             = $this->result["guest_id"];
            $data['academic_class_ids'] = array();
        }

        $data['userid'] = $userid;
        $courselist     = $this->studentcourse_model->courselist($academic_class_ids);
    ...
    public function getcourselist()
    {
        ...
        if ($role == 'student' || $role == 'parent') {
            ...
            // Use the first enrolled class for the datatable query
            $academic_class_id = !empty($academic_class_ids) ? $academic_class_ids[0] : null;
            $courselist = $this->studentcourse_model->getcourselist($academic_class_id);
        } else if ($role == 'guest') {
            $userid = $this->result["guest_id"];
            $courselist = $this->studentcourse_model->getguestcourselist();
        }
