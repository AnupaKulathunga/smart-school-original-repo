...
class Course extends Admin_Controller
{
    ...
    public function create() {
        ...
        $sectionarray = $this->input->post('class_id');
        if ($this->form_validation->run() == false) {
            $msg = array(
                'title'            => form_error('title'),
                'course_thumbnail' => form_error('course_thumbnail'),
                'description'      => form_error('description'),
                'class'            => form_error('class_id[]'),
            ...
            if (!empty($sectionarray)) {
                foreach ($sectionarray as $sectionarray_value) {
                    $sectiondata = array(
                        'course_id'        => $insert_id,
                        'academic_class_id' => $sectionarray_value,
                        'created_date'     => date('Y-m-d h:i:s'),
                    );
                    // This is used to add academic class assignment
    ...
    public function editcourse(){
        ...
        // TVET: Pass all selected academic class IDs for multi-select pre-selection
        $selected_classes = $this->course_model->selectedsection($courseID);
        $data['selected_class_ids'] = array_column($selected_classes, 'academic_class_id');
    ...
    /*This method is deprecated - TVET uses direct academic_class_id selection, no sections needed*/
    public function getsection(){
        // No longer needed in TVET architecture
        echo json_encode(array());
    }
    ...
    public function publish_unpublish(){
        ...
        $academic_class_ids     = "";
        $store_academic_class   = array();
        $class_names            = "";
        $store_class_names      = array();
        foreach ($multipalsection as $multipalsection_value) {
            if (!in_array($multipalsection_value['academic_class_id'], $store_academic_class)) {
                $store_academic_class[] = $multipalsection_value['academic_class_id'];
                $academic_class_ids .= $multipalsection_value['academic_class_id'] . ", ";
                $store_class_names[] = $multipalsection_value['class'];
                $class_names .= $multipalsection_value['class'] . ", ";
            }
        }
    ...
    public function getcourselist(){
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                $store_class_names = array();
                foreach ($multipalsection as $multipalsection_value) {
                    if (!in_array($multipalsection_value['class'], $store_class_names)) {
                        $store_class_names[] = $multipalsection_value['class'];
                        $class_names .= $multipalsection_value['class'] . ", ";
                    }
                }
    ...
    public function coursepreview()
    {
        ...
        $data['section'] = rtrim($class_names, ", ");
