...
class Coursereport extends Admin_Controller
{
    ...
    /*
    This is used to get course report data
     */
    ...
    public function getcourselist()
    {
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                $academic_classes = $this->course_model->multipalsection($value->id);

                $class_names       = "";
                $store_classes = array();
                foreach ($academic_classes as $ac_value) {
                    if (!in_array($ac_value['section'], $store_classes)) {
                        $store_classes[] = $ac_value['section'];
                        $class_names .= $ac_value['section'] . ", ";
                    }
                }

                $row       = array();
                $row[]     = $value->title;
                $row[]     = !empty($class_names) ? rtrim($class_names, ", ") : $value->class;
    ...
    public function getsellreport()
    {
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                $academic_classes = $this->course_model->multipalsection($value->online_courses_id);
    ...
    public function validation()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $users_type = $this->input->post('users_type');
            $params = array('class_id' => $class_id, 'course_id' => $course_id, 'users_type' => $users_type);
    ...
    /*
    This is used to get course list by class id
     */
    public function courselist()
    {
        $class_id   = $this->input->post('class_id');
        $users_type = $this->input->post('users_type');
        $courselist = $this->coursereport_model->courselist($class_id, $users_type);
        echo json_encode($courselist);
    }

    /*
    This is used to get course list by class id and student id
     */
    public function coursecompletelist()
    {
        ...
        if ($users_type == 'guest') {
            ...
            $studentdata = $this->coursereport_model->coursecompletereport($class_id);
        }
