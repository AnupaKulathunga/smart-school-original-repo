...
class Offlinepayment extends Admin_Controller
{
    ...
    public function checkvalidation()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $params           = array('class_id' => $class_id, 'student_id' => $student_id);
            $json_array       = array('status' => 'success', 'error' => '', 'params' => $params);
        }
        echo json_encode($json_array);
    }

    /*This is used to get course list using datatable by academic class id and student id*/
    public function courselist()
    {
        $class_id         = $this->input->post('class_id');
        $studentid        = $this->input->post('student_id');
        $coursedata       = $this->courseofflinepayment_model->courselist($class_id);
                ...
                if (empty($paidstatus) || $paidstatus['payment_type'] != 'Online') {
                    ...
                    if ($paidstatus) {
                        if ($this->rbac->hasPrivilege('online_course_offline_payment', 'can_add')) {
                            $revert = '<button data-backdrop="static" data-id=' . $value->id . ' user-data-id=' . $studentid . ' data-class-id=' . $class_id . ' class="btn btn-danger btn-xs pull-right revert_btn"><i class="fa fa-undo"> </i> ' . $this->lang->line("revert") . ' </button>';
                        } else {
                            $revert = '';
                        }
                    ...
                    <button data-backdrop="static" data-id=' . $value->id . ' user-data-id=' . $studentid . '  class="btn btn-primary btn-xs pull-right print_btn"><i class="fa fa-print"></i> ' . $this->lang->line("print") . '</button>';
                        ...
                        if ($this->rbac->hasPrivilege('online_course_offline_payment', 'can_add')) {
                            $row[] = '<button data-backdrop="static" data-id=' . $value->id . ' user-data-id=' . $studentid . ' data-class-id=' . $class_id . ' data-keyboard="false" data-toggle="modal" data-target="#payment_modal" class="btn-success pull-right btn-xs paid_btn"><i class="fa fa-money"></i> ' . $this->lang->line("pay") . '</button>';
                        } else {
                            $row[] = '';
                        }
                    }
    ...
    /*This is used to get student list by class*/
    ...
    /*This is used to show course list by class*/
    public function search()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $class_id   = $this->input->post('class_id');
            $student_id = $this->input->post('student_id');
            $courselist = $this->courseofflinepayment_model->courselist($class_id);
    ...
    public function success()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $params = array('class_id' => $class_id, 'student_id' => $student_id);
            $array  = array('status' => 'success', 'student_id' => $student_id, 'class_id' => $class_id, 'error' => '', 'message' => $this->lang->line('success_message'), 'params' => $params);
        }
    ...
    public function revert()
    {
        ...
        $params     = array('class_id' => $class_id, 'student_id' => $studentid);
        $json_array = array('status' => 'success', 'error' => '', 'params' => $params);
        echo json_encode($json_array);
