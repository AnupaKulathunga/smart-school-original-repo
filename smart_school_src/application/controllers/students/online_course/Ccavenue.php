    ...
    public function success() 
    {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if(!empty($params['courseid'])) {
                ...
                $sender_details = array('email'=>$params['email'], 'courseid'=>$params['courseid'],'class' => $params['class'],  'academic_class_ids'=> $params['class_sections'], 'title' => $params['course_name'], 'price' => $params['total_amount'], 'discount' => $params['discount'], 'assign_teacher' => $params['staff'], 'purchase_date' => $this->customlib->dateformat(date('Y-m-d')));
    ...
    public function guestsuccess() {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            foreach ($params as $cart_data_value) {
                ...
                $sender_details = array('email'=>$cart_data_value['email'], 'courseid'=> $cart_data_value['id'],'class' => null,  'academic_class_ids'=> null, 'title' => $cart_data_value['name'], 'price' => $cart_data_value['price'], 'discount' => $cart_data_value['discount'], 'assign_teacher' => $cart_data_value['staff'], 'purchase_date' => $this->customlib->dateformat(date('Y-m-d')));
            ...
            redirect(base_url("students/online_course/course_payment/paymentsuccess"));
            // if(!empty($params['courseid'])) {
            //     $sender_details = array('courseid'=>$params['courseid'],'class' => $params['class'],  'academic_class_ids'=> $params['class_sections'], 'title' => $params['course_name'], 'price' => $params['total_amount'], 'discount' => $params['discount'], 'assign_teacher' => $params['staff'], 'paid_free' => $params['paid_free'], 'purchase_date' => date('Y-m-d'));         
