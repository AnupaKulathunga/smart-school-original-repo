...
class Studentfee_model extends MY_Model {
    ...
    public function getStudentFees($id = null) {
        ...
        $this->db->join('academic_class_enrolment as enrolment', 'enrolment.id = student_fees.student_session_id');
        ...
        $this->db->where('enrolment.student_id', $id);
    ...
    public function getTodayStudentFees() {
        $this->db->select('student_fees.date,student_fees.id,student_fees.amount,student_fees.amount_discount,student_fees.amount_fine,student_fees.created_at,CONCAT(academic_class.class_code, " - ", academic_class.cohort_name) as `class`,"" as `section`,students.firstname,students.middlename,students.lastname,students.admission_no,students.roll_no,students.dob,students.guardian_name,feetype.type', FALSE)->from('student_fees');
        $this->db->join('academic_class_enrolment as enrolment', 'enrolment.id = student_fees.student_session_id');
        $this->db->join('feemasters', 'feemasters.id = student_fees.feemaster_id');
        $this->db->join('feetype', 'feetype.id = feemasters.feetype_id');
        $this->db->join('academic_class', 'academic_class.id = enrolment.class_id');
         ...
         if (!empty($feegroups))
  {
       ...
       return $result_value2;
        ...
        $query = "SELECT student_fees_master.amount as `previous_balance_amount`,IFNULL(student_fees_deposite.id, 0) as student_fees_deposite_id, IFNULL(student_fees_deposite.fee_groups_feetype_id, 0) as fee_groups_feetype_id, IFNULL(student_fees_deposite.amount_detail, 0) as amount_detail, student_fees_master.id as `fee_master_id`,fee_groups_feetype.feetype_id,fee_groups_feetype.amount,fee_groups_feetype.due_date, `academic_class`.`id` AS `class_id`, `enrolment`.`id` as `student_session_id`, `students`.`id`, CONCAT(`academic_class`.`class_code`, ' - ', `academic_class`.`cohort_name`) as `class`, '' AS `section_id`, '' as `section`, `students`.`id`, `students`.`admission_no`, `students`.`roll_no`, `students`.`admission_date`, `students`.`firstname`,`students`.`middlename`, `students`.`lastname`, `students`.`image`, `students`.`mobileno`, `students`.`email`, `students`.`state`, `students`.`city`, `students`.`pincode`, `students`.`religion`, `students`.`dob`, `students`.`current_address`, `students`.`permanent_address`, IFNULL(students.category_id, 0) as `category_id`, IFNULL(categories.category, '') as `category`, `students`.`adhar_no`, `students`.`samagra_id`, `students`.`bank_account_no`, `students`.`bank_name`, `students`.`ifsc_code`, `students`.`guardian_name`, `students`.`guardian_relation`, `students`.`guardian_phone`, `students`.`guardian_address`, `students`.`is_active`, `students`.`created_at`, `students`.`updated_at`, `students`.`father_name`, `students`.`rte`, `students`.`gender`,fee_groups.name as `fee_group`,feetype.type as `fee_type`,feetype.code as `fee_code`, fee_groups.is_system FROM `student_fees_master` INNER JOIN fee_session_groups on fee_session_groups.id= student_fees_master.fee_session_group_id INNER JOIN fee_groups on fee_groups.id=fee_session_groups.fee_groups_id INNER JOIN fee_groups_feetype on fee_groups_feetype.fee_session_group_id=student_fees_master.fee_session_group_id INNER JOIN feetype on feetype.id=fee_groups_feetype.feetype_id LEFT JOIN student_fees_deposite on student_fees_deposite.student_fees_master_id=student_fees_master.id and student_fees_deposite.fee_groups_feetype_id=fee_groups_feetype.id INNER JOIN academic_class_enrolment as enrolment on enrolment.id=student_fees_master.student_session_id INNER JOIN students on students.id=enrolment.student_id JOIN `academic_class` ON `enrolment`.`class_id` = `academic_class`.`id` LEFT JOIN `categories` ON `students`.`category_id` = `categories`.`id` WHERE `students`.`is_active` = 'yes' and academic_class.session_id = ".$this->current_session."  ".$where_condition_string." ORDER BY student_fees_master.id asc";
        ...
        `students`.`admission_no`, `students`.`roll_no`, `students`.`admission_date`, `students`.`firstname`,`students`.`middlename`, `students`.`lastname`, `students`.`image`, `students`.`mobileno`, `students`.`email`, `students`.`state`, `students`.`city`, `students`.`pincode`, `students`.`religion`,  `students`.`dob`, `students`.`current_address`, `students`.`permanent_address`,  `students`.`adhar_no`,  `students`.`samagra_id`, `students`.`bank_account_no`,  `students`.`bank_name`, `students`.`ifsc_code`, `students`.`guardian_name`,  `students`.`guardian_relation`, `students`.`guardian_phone`, `students`.`guardian_address`, `students`.`is_active`,  `students`.`created_at`, `students`.`updated_at`,  `students`.`father_name`,  `students`.`rte`, `students`.`gender`, IFNULL(students.category_id, 0) as `category_id`, IFNULL(categories.category,"") as `category`, enrolment.class_id,CONCAT(academic_class.class_code, " - ", academic_class.cohort_name) as `class`,"" as `section`,"" as `section_id`,enrolment.student_id,"" as fee_group,"Transport Fees" as name, "Transport Fees" as `fee_type`, transport_feemaster.month as `fee_code`,0 as is_system,student_transport_fees.student_session_id,
             `enrolment`.`id` as `student_session_id`,0 as is_system', FALSE)->from('student_transport_fees');
        $this->db->join('student_fees_deposite', 'student_transport_fees.id = `student_fees_deposite`.`student_transport_fee_id`','left');
        $this->db->join('transport_feemaster', '`student_transport_fees`.`transport_feemaster_id` = `transport_feemaster`.`id`','left');
        $this->db->join('academic_class_enrolment as enrolment', 'enrolment.id = `student_transport_fees`.`student_session_id`', 'INNER');
        $this->db->join('route_pickup_point', 'route_pickup_point.id = student_transport_fees.route_pickup_point_id');
    ...
    public function getDueStudentFees($feegroup_id = null, $fee_groups_feetype_id = null, $class_id = NULL, $section_id = NULL) {
        ...
        $query = "SELECT IFNULL(student_fees_deposite.id, 0) as student_fees_deposite_id, IFNULL(student_fees_deposite.fee_groups_feetype_id, 0) as fee_groups_feetype_id, IFNULL(student_fees_deposite.amount_detail, 0) as amount_detail, student_fees_master.id as `fee_master_id`,fee_groups_feetype.feetype_id,fee_groups_feetype.amount,fee_groups_feetype.due_date, `academic_class`.`id` AS `class_id`, `enrolment`.`id` as `student_session_id`, `students`.`id`, CONCAT(`academic_class`.`class_code`, ' - ', `academic_class`.`cohort_name`) as `class`, '' AS `section_id`, '' as `section`, `students`.`id`, `students`.`admission_no`, `students`.`roll_no`, `students`.`admission_date`, `students`.`firstname`,`students`.`middlename`, `students`.`lastname`, `students`.`image`, `students`.`mobileno`, `students`.`email`, `students`.`state`, `students`.`city`, `students`.`pincode`, `students`.`religion`, `students`.`dob`, `students`.`current_address`, `students`.`permanent_address`, IFNULL(students.category_id, 0) as `category_id`, IFNULL(categories.category, '') as `category`, `students`.`adhar_no`, `students`.`samagra_id`, `students`.`bank_account_no`, `students`.`bank_name`, `students`.`ifsc_code`, `students`.`guardian_name`, `students`.`guardian_relation`, `students`.`guardian_phone`, `students`.`guardian_address`, `students`.`is_active`, `students`.`created_at`, `students`.`updated_at`, `students`.`father_name`, `students`.`rte`, `students`.`gender` FROM `students` JOIN `academic_class_enrolment` AS `enrolment` ON `enrolment`.`student_id` = `students`.`id` JOIN `academic_class` ON `enrolment`.`class_id` = `academic_class`.`id` LEFT JOIN `categories` ON `students`.`category_id` = `categories`.`id` INNER JOIN student_fees_master on student_fees_master.student_session_id=enrolment.id and student_fees_master.fee_session_group_id=" . $this->db->escape($feegroup_id) . " LEFT JOIN student_fees_deposite on student_fees_deposite.student_fees_master_id=student_fees_master.id and student_fees_deposite.fee_groups_feetype_id=" . $this->db->escape($fee_groups_feetype_id) . " INNER JOIN fee_groups_feetype on fee_groups_feetype.id = " . $this->db->escape($fee_groups_feetype_id) . " WHERE `academic_class`.`session_id` = " . $this->current_session . " AND
    ...
    public function getFeesByClass($class_id = null, $section_id = null, $student_id = null) {
        $query = "SELECT feemasters.id as feemastersid, feemasters.amount as amount,IFNULL(student_fees.id, 'xxx') as invoiceno,IFNULL(student_fees.amount_discount, 'xxx') as discount,IFNULL(student_fees.amount_fine, 'xxx') as fine, IFNULL(student_fees.date, 'xxx') as date,feetype.type,feecategory.category FROM feemasters LEFT JOIN (select student_fees.id,student_fees.feemaster_id,student_fees.amount_fine,student_fees.amount_discount,student_fees.date,student_fees.student_session_id from student_fees, academic_class_enrolment as enrolment where student_fees.student_session_id=enrolment.id and enrolment.student_id=" . $this->db->escape($student_id) . " and enrolment.class_id=" . $this->db->escape($class_id) . ") as student_fees ON student_fees.feemaster_id=feemasters.id LEFT JOIN feetype ON feemasters.feetype_id = feetype.id LEFT JOIN feecategory ON feetype.feecategory_id = feecategory.id where feemasters.class_id=" . $this->db->escape($class_id) . " and feemasters.session_id=" . $this->db->escape($this->current_session);
        $query = $this->db->query($query);
        return $query->result_array();
    }
    ...
    public function getFeeBetweenDate($start_date, $end_date) {
        ...
        $this->db->select('student_fees.date,student_fees.id,student_fees.amount,student_fees.amount_discount,student_fees.amount_fine,student_fees.created_at,students.rte,CONCAT(academic_class.class_code, " - ", academic_class.cohort_name) as `class`,"" as `section`,students.firstname,students.middlename,students.lastname,students.admission_no,students.roll_no,students.dob,students.guardian_name,feetype.type', FALSE)->from('student_fees');
        $this->db->join('academic_class_enrolment as enrolment', 'enrolment.id = student_fees.student_session_id');
        $this->db->join('feemasters', 'feemasters.id = student_fees.feemaster_id');
        $this->db->join('feetype', 'feetype.id = feemasters.feetype_id');
        $this->db->join('academic_class', 'academic_class.id = enrolment.class_id');
