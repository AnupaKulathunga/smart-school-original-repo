 ...
 * Uses 'name' field for assessment title and 'status' for publication status
 */
class Academic_assessment_model extends CI_Model
{
    ...
    {
        $this->db->select('a.*, a.name as title,
                          CASE WHEN a.status = "Published" OR a.status = "Active" THEN 1 ELSE 0 END as is_published,
                          c.class_code, c.cohort_name,
                          s.name as subject_name, s.code as subject_code,
                          l.code as level_code,
                          st.name as creator_name, st.surname as creator_surname,
                          (SELECT COUNT(*) FROM academic_assessment_marks WHERE assessment_id = a.id) as marks_count');
        ...
        if (isset($filters['is_published']) && $filters['is_published']) {
            $this->db->where_in('a.status', ['Published', 'Active']);
        }
    ...
    {
        return $this->db->select('a.*, a.name as title,
                                  CASE WHEN a.status = "Published" OR a.status = "Active" THEN 1 ELSE 0 END as is_published,
                                  c.class_code, c.cohort_name, c.primary_lecturer_id,
                                  s.name as subject_name, s.code as subject_code,
    ...
    public function getByClass($class_id, $published_only = false)
    {
        $this->db->select('a.*, a.name as title,
                          CASE WHEN a.status = "Published" OR a.status = "Active" THEN 1 ELSE 0 END as is_published,
                          c.class_code, s.name as subject_name, l.code as level_code');
        $this->db->from($this->table . ' a');
        $this->db->join('academic_class c', 'c.id = a.class_id');
        $this->db->join('academic_subject_level sl', 'sl.id = c.subject_level_id');
        $this->db->join('academic_subject s', 's.id = sl.subject_id');
    ...
    public function update($id, $data)
    {
        ...
        if (isset($data['title'])) $update_data['name'] = $data['title'];
    ...
    {
        return $this->update($id, array('status' => 'Published'));
    }
    ...
    {
        return $this->update($id, array('status' => 'Draft'));
    }
    ...
    {
        $this->db->select('a.*, a.name as title, c.class_code, s.name as subject_name, l.code as level_code');
        $this->db->from($this->table . ' a');
        $this->db->join('academic_class c', 'c.id = a.class_id');
        $this->db->join('academic_subject_level sl', 'sl.id = c.subject_level_id');
        $this->db->join('academic_subject s', 's.id = sl.subject_id');
        ...
        $this->db->join('student_session ss', 'ss.id = e.student_session_id');
        $this->db->where('ss.student_id', $student_id);
        $this->db->where('e.status', 'Enrolled');
        $this->db->where_in('a.status', ['Published', 'Active']);
