...
class Feegrouptype_model extends MY_Model
{
   ...
   public function getFeeGroupByIDAndStudentSessionID($id,$student_session_id)
    ...
    // TVET: Replaced student_session/classes with academic_class_enrolment/academic_class
    public function getFeeTypeStudents($fee_session_group_id, $fee_groups_feetype_id)
    {
        $query = "SELECT student_fees_master.*,student_fees_deposite.student_fees_master_id,student_fees_deposite.fee_groups_feetype_id,student_fees_deposite.amount_detail, students.id as `student_id`, students.roll_no,students.admission_date,students.firstname,students.middlename, students.lastname,students.image, students.mobileno, students.email ,students.state , students.city , students.pincode , students.religion,students.dob ,students.current_address, students.permanent_address,students.category_id, IFNULL(categories.category, '') as `category`, students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name, students.guardian_relation,students.guardian_phone,students.guardian_email, ac.class_code as `class`, students.guardian_address,students.is_active,`students`.`father_name`,`students`.`app_key`,`students`.`parent_app_key`,`students`.`gender` FROM `student_fees_master` LEFT JOIN student_fees_deposite on student_fees_deposite.student_fees_master_id=student_fees_master.id and student_fees_deposite.fee_groups_feetype_id = " . $this->db->escape($fee_groups_feetype_id) . " INNER JOIN academic_class_enrolment e on e.id=student_fees_master.student_session_id INNER JOIN students on students.id=e.student_id INNER JOIN academic_class ac ON ac.id = e.class_id LEFT JOIN `categories` ON `students`.`category_id` = `categories`.`id` WHERE e.status='Active' and student_fees_master.fee_session_group_id =" . $this->db->escape($fee_session_group_id);
        $query                = $this->db->query($query);
        $fee_group_type_array = $query->result();
        return $fee_group_type_array;
    }
