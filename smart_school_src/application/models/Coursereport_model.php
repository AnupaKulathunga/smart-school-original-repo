...
class Coursereport_model extends MY_Model {
    ...
    public function get_teacherstudents()
    {
        ...
        $class_section_array = $this->customlib->get_myClassSection();
          if (!empty($class_section_array)) {
            ...
            $this->db->join('academic_class_enrolment ace', 'ace.student_session_id = student_session.id');
            $this->db->group_start();
            foreach ($class_section_array as $class_sectionkey => $class_sectionvalue) {
                foreach ($class_sectionvalue as $class_sectionvaluekey => $class_sectionvaluevalue) {
                    $this->db->or_where('ace.class_id', $class_sectionvaluevalue);
                }
            }
            $this->db->group_end();
            $query  = $this->db->get();
    ...
    public function sellreport()
    {
        ...
        $class_section_array = $this->customlib->get_myClassSection();
        $this->datatables
            ->select('count(online_courses.id) as sell_count, online_courses.title, online_courses.created_by, staff.name, staff.surname, staff.employee_id, online_course_payment.date,online_course_payment.online_courses_id,CONCAT(academic_subject.name, " - ", academic_level.code) as class,s.name as assign_name,s.surname as assign_surname,s.employee_id as assign_employee_id,staff_roles.role_id', FALSE)
            ->searchable('online_courses.title, online_courses.created_by, staff.name, staff.surname, online_course_payment.date,online_course_payment.online_courses_id')
             ->orderable('online_courses.title,class," ",sell_count,assign_name, name')
            ...
            ->join('online_course_academic_class', 'online_course_academic_class.course_id = online_courses.id')
            ->join('academic_class', 'academic_class.id = online_course_academic_class.academic_class_id')
            ->join('academic_subject_level', 'academic_subject_level.id = academic_class.subject_level_id')
            ->join('academic_subject', 'academic_subject.id = academic_subject_level.subject_id')
            ->join('academic_level', 'academic_level.id = academic_subject_level.level_id');
            if (!empty($class_section_array)) {
            $this->datatables->group_start();

            foreach ($class_section_array as $class_sectionkey => $class_sectionvalue) {
                foreach ($class_sectionvalue as $class_sectionvaluekey => $class_sectionvaluevalue) {
                    $this->datatables->or_where('online_course_academic_class.academic_class_id', $class_sectionvaluevalue);
                }
            }
    ...
    public function trendingreport() {
        ...
        $this->datatables
            ->select('online_courses.*,staff.name,staff.surname,staff.employee_id,s.name as assign_name,s.surname as assign_surname,s.employee_id as assign_employee_id,CONCAT(academic_subject.name, " - ", academic_level.code) as class,staff_roles.role_id', FALSE)
            ->searchable('online_courses.title, online_courses.created_by, staff.name, staff.surname,view_count')
            ->orderable('online_courses.title,class," ",view_count, online_courses.created_by, staff.name, staff.surname')
            ->join('staff','staff.id=online_courses.created_by')
            ->join('staff as s', 's.id = online_courses.teacher_id')
            ->join('staff_roles','staff_roles.staff_id=staff.id')
            ->join('online_course_academic_class', 'online_course_academic_class.course_id = online_courses.id')
    ...
    public function courselist($academic_class_id, $users_type = null)
    {
        if($users_type == 'guest'){
            ...
            $this->db->where('online_course_academic_class.academic_class_id',$academic_class_id);
        }
    ...
    /*This is used to get student list by academic_class_id and course id*/
    public function coursecompletereport($academic_class_id)
    {
        $this->datatables
            ...
            ->join('academic_class_enrolment', 'academic_class_enrolment.class_id = academic_class.id')
            ->join('student_session', 'student_session.id = academic_class_enrolment.student_session_id')
            ->join('students', 'student_session.student_id = students.id')
            ->from('academic_class')
            ->where(array('academic_class.id' => $academic_class_id));
            return $this->datatables->generate('json');
    }
    ...
    public function get_online_course_students($exam_id,$course_id)
    {
       ...
       $query=" SELECT count(case when online_courses.free_course=1 then 1 end) as total_free_course, count(case when online_courses.free_course=0 and (select online_course_payment.student_id from online_course_payment where online_course_payment.student_id=ss.student_id and online_course_payment.online_courses_id=`oca`.`course_id`) then 1 end) as total_paid_course FROM `online_course_academic_class` `oca` JOIN `academic_class` `ac` ON `ac`.`id` = `oca`.`academic_class_id` JOIN `academic_class_enrolment` `ace` ON `ace`.`class_id` = `ac`.`id` JOIN `student_session` `ss` ON `ss`.`id` = `ace`.`student_session_id` left join online_courses on online_courses.id=oca.course_id WHERE `oca`.`course_id` = $course_id AND `ss`.`session_id` = $this->current_session";
