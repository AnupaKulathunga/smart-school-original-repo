...
class Studentcourse_model extends MY_Model {
    ...
    /*
    This is used to show course list by academic class
    */
    public function courselist($academic_class_ids=null) {
        ...
        $query_string="" ;
        if(!empty($academic_class_ids)){

            $query_string = ', CONCAT(subj.name, " - ", lvl.code) as class' ;
            $this->db->join('online_course_academic_class', 'online_course_academic_class.course_id = online_courses.id','left');
            $this->db->join('academic_class ac', 'ac.id = online_course_academic_class.academic_class_id','left');
            $this->db->join('academic_subject_level asl', 'asl.id = ac.subject_level_id','left');
            $this->db->join('academic_subject subj', 'subj.id = asl.subject_id','left');
            $this->db->join('academic_level lvl', 'lvl.id = asl.level_id','left');
            $this->db->group_by('online_course_academic_class.course_id');
            $this->db->where_in('online_course_academic_class.academic_class_id', $academic_class_ids);
        }

        $this->db->select('online_courses.*,staff.name,staff.surname,staff.image,staff.gender,staff.employee_id'.$query_string, FALSE)->from('online_courses');
    ...
    This is used to show course list by academic class
    */
    public function getcourselist($academic_class_id=null) {

        $join_string="";$select_string="";

        $join_string = " join online_course_academic_class on online_course_academic_class.course_id = online_courses.id join academic_class ac on ac.id = online_course_academic_class.academic_class_id join academic_subject_level asl on asl.id = ac.subject_level_id join academic_subject subj on subj.id = asl.subject_id join academic_level lvl on lvl.id = asl.level_id" ;

        $select_string = "CONCAT(subj.name, ' - ', lvl.code) as `class`,";

        $query   =   "select online_courses.*, ".$select_string." staff.name,staff.surname,staff.image from online_courses join staff on staff.id = online_courses.teacher_id ".$join_string."  where 1=1 and online_course_academic_class.academic_class_id = $academic_class_id and online_courses.status = '1'   " ;
    ...
    public function getguestcourselist() {
        ...
        $query   =   "select online_courses.*, staff.name,staff.surname,staff.image from online_courses join staff on staff.id = online_courses.teacher_id  where 1=1 ".$condition." and online_courses.status = '1' group by online_courses.id " ;
    ...
    */
    ...
    /*
    This is used to show purchased course list for guest
    */
